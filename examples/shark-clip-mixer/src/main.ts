...
const ctx = await createExample("Shark Clip Mixer");
...
for (let index = 0; index < sharkCount; index++) {
  ...
  const clip = getClipForLane(lane);
  const meta = {
    label: `shark-${index}`,
    lane,
    clip,
  ...
  const id = createShark(transform, meta, { clip });
  ids.push(id);
}
...
ctx.panel.button("cycle selected clip", () => {
  if (!selected || clipNames.length <= 1) {
    return;
  }
  const meta = sharks.getMetadata(selected);
  if (!meta) {
    return;
  }
  setSharkClip(selected, getNextClip(meta.clip));
});

ctx.panel.button("cycle selected lane", () => {
  if (!selected || clipNames.length <= 1) {
    return;
  }
  const meta = sharks.getMetadata(selected);
  ...
  const nextClip = getNextClip(meta.clip);
  for (const id of ids) {
...
onBeforeRender(ctx.scene, (deltaMs) => {
  ...
  ctx.panel.set("clips", clipNames.length);
  ctx.panel.set("phase mode", vatSet ? "clip + phase buckets" : "source animation");
...
function createShark(transform: Mat4, metadata: SharkMeta, options: { clip?: string } = {}): InstanceId {
  if (vatSet) {
...
function applyVatPhaseBuckets(): void {
  ...
  for (let index = 0; index < ids.length; index++) {
    ...
    const meta = sharks.getMetadata(id);
    const clip = meta ? vatSet.clips[meta.clip] : vatSet.getActiveClip();
    if (!clip) {
      continue;
    }
...
function applyLaneClips(): void {
  if (!vatSet || clipNames.length === 0) {
    return;
  }
  for (const id of ids) {
    const meta = sharks.getMetadata(id);
    ...
    setSharkClip(id, getClipForLane(meta.lane));
  }
  applyVatPhaseBuckets();
}
...
function getClipForLane(lane: number): string {
  if (clipNames.length === 0) {
    return "";
  }
  return clipNames[(activeAnimationIndex + lane) % clipNames.length] ?? clipNames[0] ?? "";
}

function getNextClip(clip: string): string {
  if (clipNames.length === 0) {
    return clip;
  }
  const index = clipNames.indexOf(clip);
  return clipNames[(Math.max(0, index) + 1) % clipNames.length] ?? clip;
}
