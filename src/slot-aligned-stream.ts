/** A contiguous range of dirty instance slots. Both endpoints are inclusive. */
...
/** Diagnostic counters shared by Lite and future Babylon.js stream adapters. */
...
/**
 * Engine-neutral, capacity-owned float data that follows compact renderer slots.
 * The owner is responsible for forwarding its slot lifecycle to this object.
 */
export class SlotAlignedFloatStream {
  ...
  setSlot(slot: number, value: ArrayLike<number>): void {
    this.#assertSlot(slot);
    if (value.length !== this.components) {
      throw new Error("SlotAlignedFloatStream value length must match components.");
    }
    const offset = slot * this.components;
    for (let component = 0; component < this.components; component++) {
      this.#data[offset + component] = value[component] ?? 0;
    }
    this.#dirty.add(slot);
    this.stats.slotWrites++;
  }
  ...
  truncate(count: number): void {
    ...
    if (!Number.isInteger(count) || count < 0 || count > this.#capacity) {
      throw new Error("Invalid SlotAlignedFloatStream live count.");
    }
    for (const slot of this.#dirty) {
      if (slot >= count) this.#dirty.delete(slot);
    }
  }
  ...
  markAllDirty(count: number): void {
    this.#assertUsable();
    for (let slot = 0; slot < count; slot++) this.#dirty.add(slot);
  }

  dirtyRanges(): SlotDirtyRange[] {
    return coalesceDirtySlots(this.#dirty);
  }
  ...
  #assertUsable(): void {
    if (this.#disposed) throw new Error("SlotAlignedFloatStream is disposed.");
  }
}
...
export function coalesceDirtySlots(slots: Iterable<number>): SlotDirtyRange[] {
  ...
  for (const slot of sorted) {
    ...
    if (last && slot === last.end + 1) {
      ranges[ranges.length - 1] = { start: last.start, end: slot };
    } else {
...
interface SlotStreamHost {
  capacity: number;
  count: number;
  streams: Set<SlotAlignedFloatStream>;
}
