...
export function copyMatrix16(source: Float32Array, offset: number, target: Float32Array): void {
  for (let index = 0; index < 16; index++) {
    target[index] = source[offset + index] ?? 0;
  }
}
...
export function swapMatrix16(buffer: Float32Array, aOffset: number, bOffset: number, scratch: Float32Array): void {
  ...
  for (let index = 0; index < 16; index++) {
    buffer[bOffset + index] = scratch[index] ?? 0;
  }
}
...
export function writeZeroMatrixScale(buffer: Float32Array, offset: number): void {
  const x = buffer[offset + 12] ?? 0;
  const y = buffer[offset + 13] ?? 0;
  const z = buffer[offset + 14] ?? 0;
  for (let index = 0; index < 16; index++) {
