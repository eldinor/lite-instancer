...
import { registerPickingSupport } from "./babylon-registration.js";
import type { BaseInstanceSet, InstanceId } from "./types.js";

registerPickingSupport();

export type PickableInstanceSet = Pick<BaseInstanceSet<unknown>, "getIdForSlot">;

/** Result after mapping a Babylon.js thin-instance pick back to a stable instance ID. */
...
/** Minimal shape accepted from Babylon.js picking results. */
...
/**
 * Maps Babylon.js `mesh + thinInstanceIndex` picks back to stable `InstanceId` values.
 *
...
export class PickingRegistry {
  ...
  #previousPicking = new Map<Mesh, boolean>();

  /** Associate a backing mesh with an instance set. */
  register(mesh: Mesh, set: PickableInstanceSet): this {
    if (!this.#previousPicking.has(mesh)) {
      this.#previousPicking.set(mesh, mesh.thinInstanceEnablePicking);
    }
  ...
  unregister(mesh: Mesh): boolean {
    const removed = this.#meshToSet.delete(mesh);
    if (removed) {
      mesh.thinInstanceEnablePicking = this.#previousPicking.get(mesh) ?? false;
      this.#previousPicking.delete(mesh);
    }
    return removed;
  }
  ...
  clear(): void {
    for (const mesh of this.#meshToSet.keys()) {
      mesh.thinInstanceEnablePicking = this.#previousPicking.get(mesh) ?? false;
    }
    this.#meshToSet.clear();
    this.#previousPicking.clear();
  }
  ...
  /** Resolve a Babylon.js pick result to a stable ID. */
