...
- Require Babylon Lite 1.31 or newer within the 1.x release line and drop compatibility with the former Lite 1.14 private text-storage contract.
...
- Add lifecycle-managed Sprite2D buckets per annotation `zIndex`, with lines
  ...
  bucket removal.
- Add nine-slice GPU label backgrounds with background/border colors, width,
  radius, padding-aware measurement, cached atlas patches, one background draw
  call per z bucket, lifecycle statistics, and a focused gallery demo.
