...
const interaction = await readFile(new URL("../dist/interaction.js", import.meta.url), "utf8");
const declaration = await readFile(new URL("../dist/textrender.d.ts", import.meta.url), "utf8");
const interactionDeclaration = await readFile(new URL("../dist/interaction.d.ts", import.meta.url), "utf8");
...
assert(packageJson.exports?.["./interaction"]?.import === "./dist/interaction.js", "Missing interaction JS export");
assert(packageJson.exports?.["./interaction"]?.types === "./dist/interaction.d.ts", "Missing interaction type export");
...
assert(interaction.includes("createAnnotationInteractionManager"), "interaction bundle is missing its factory");
assert(!interaction.includes("createTextRenderer"), "interaction bundle contains TextRenderer implementation");
assert(interactionDeclaration.includes("createAnnotationInteractionManager"), "interaction declaration is missing its factory");
